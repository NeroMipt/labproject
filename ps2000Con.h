...
int16_t values_b [BUFFER_SIZE]; // block mode bu#include "qcustomplot.h"ffer, Channel B
 ...
 ****************************************************************************/
void collect_block_immediate (double *Time_axisX, double *VoltageA_axisY, double *VoltageB_axisY)
{
	int32_t		i;
...
void collect_fast_streaming_triggered (void)
{
	...
	printf ( "Press a key to start\n" );
    _getch ();		printf ( "Device opened successfully\n\n" );
    //get_info ();
	...
	*/
	...
	{
        ps2000_get_streaming_last_values (unitOpened.handle, ps2000FastStreamingReady);		printf ( "Device opened successfully\n\n" );
        //get_info ();
